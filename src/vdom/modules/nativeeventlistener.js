// ! eventlistener的一份拷贝, 只不过将on全部改为nativOn
// ! 由于没有处理modifier, 所以只对nativeOn进行处理
...
function updateEventListeners(oldVnode, vnode) {
  const oldNativeOn = oldVnode.data.nativeOn
  const oldListener = oldVnode.listener
  const oldElm = oldVnode.elm
  // c = a && b相当于 c = a ? b : a
  const nativeOn = vnode && vnode.data.nativeOn
  const elm = vnode && vnode.elm

  // 如果两者on相同, 则代表
  // 1. oldVnode与新vnode相同, 至少我是这样认为的, 源码中说是为了复用
  // 2. oldOn = on = undefined
  if (oldNativeOn === nativeOn) {
    return
  }

  if (oldNativeOn && oldListener) {
    ...
    if (!nativeOn) {
       ...
       * * 值指向其绑定的vnode
       */
      for (let name in nativeOn) {
        oldElm.removeListener(name, oldListener)
      }
    } else {
      for (let name in oldNativeOn) {
        // 如果on不为空, 则代表设置了新的事件
...
function handleEvent(event, vnode) {
  // 判断事件类型, 根据事件类型去nativeOn中调用相应的
  const name = event.type
  const nativeOn = vnode.data.nativeOn

  // 如果on存在, 且on注册了(包含有键名)事件event.type
  if (nativeOn && nativeOn[name]) {
    invokeHandler(nativeOn[name], vnode, event)
  }
}
...
export const nativeEventListenerModule = {
